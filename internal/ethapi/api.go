...
// Call Get Reserve function signature for getReserve: 0x0902f1ac
func Trace_sloadTracer(ctx context.Context, cl *ethclient.Client, Address common.Address) json.RawMessage {
	// Get RPC
	...
	_gas := hexutil.Uint64(4294967295)
	_callArgs := CallArgs{
		From: &Address,
		To:   &Address,
		Gas:  &_gas,
	...
	_Tracer := map[string]string{
		"Tracer": "sloadTracer",
	}
	var raw json.RawMessage
	err := rpcClient.CallContext(context.Background(), &raw, "debug_traceCall", _callArgs, "latest", _Tracer)
	if err != nil {
...
func FetchUniswapV2LpAttribs(ctx context.Context, cl *ethclient.Client, Address common.Address) json.RawMessage {
	// Get RPC
	var clientValue = reflect.ValueOf(cl).Elem()
	fieldStruct := clientValue.FieldByName("c")
	clientPointer := reflect.NewAt(fieldStruct.Type(), unsafe.Pointer(fieldStruct.UnsafeAddr())).Elem()
	rpcClient, _ := clientPointer.Interface().(*rpc.Client)
	var raw json.RawMessage
	err := rpcClient.CallContext(context.Background(), &raw, "debug_fetchUniswapV2LpAttribs", "latest", Address)
	if err != nil {
		log.Println("Error: ", err.Error())
		return nil
	}
	...
	var (
		start = time.Now()
		_res = FetchUniswapV2LpAttribs(ctx, cl, common.HexToAddress("0xDCbc1D9D48016b8d5F3B0F9045Eb3B72F38E6B93"))
		log.Println("_res: ", string(_res), " Time Taken: ", time.Since(start))
	)

	var (
		start = time.Now()
		_Addresses := []common.Address{
		common.HexToAddress("0xDCbc1D9D48016b8d5F3B0F9045Eb3B72F38E6B93"),
		common.HexToAddress("0xbe98c84b7d75891d58823999a0dd3414cdcba0ad"),
		common.HexToAddress("0xd46e7f33f8788f87d6017074dc4e4d781d3df91e"),
		common.HexToAddress("0xcb769432905d83052caaa07bdc88ad434edf0790"),
		common.HexToAddress("0x3fa61ef67697d93aca2f23ef532e6cbb2c50eb5f"),
		common.HexToAddress("0x1642bAf29eFcF0d5d30c350fB11CB4A849B63cc6"),
		common.HexToAddress("0x756ed14d42475b1ee7cf1c5ba035e3d1a9295061"),
		common.HexToAddress("0x42981d0bfbAf196529376EE702F2a9Eb9092fcB5"),
		common.HexToAddress("0x55d398326f99059fF775485246999027B3197955")}
		_res = FetchMultiUniswapV2LpAttribs(ctx, cl, _Addresses)
		log.Println("_res: ", string(_res), " Time Taken: ", time.Since(start))
	)

	var (
		start = time.Now()
		_res = GetUniswapV2LpAttribs(ctx, cl, common.HexToAddress("0xbE98c84B7D75891d58823999a0DD3414CDcbA0Ad"))
		log.Println("Attributes: ", string(_res), " Time Taken: ", time.Since(start))
	)

	var(
		start := time.Now()
		// 0x87D7fd8c446Cb5D3da3CA23f429e7b7504d1931C SafeMoon Pancake V2 Lp
		_res := GetUniswapV2LpAttribs(ctx, cl, common.HexToAddress("0x87D7fd8c446Cb5D3da3CA23f429e7b7504d1931C"))
		log.Println("Attributes: ", string(_res), " Time Taken: ", time.Since(start))
	)
...
func (s *PublicBlockChainAPI) fetchUniswapV2LpAttribs(ctx context.Context, args CallArgs, state *state.StateDB, header *types.Header) (hexutil.Bytes, error) {
	defer func(start time.Time) {
		log.Debug("Executing FetchUniswapV2LpAttribs call finished", "runtime", time.Since(start))
	}(time.Now())

	// Get a new instance of the EVM.
	msg := args.ToMessage(s.b.RPCGasCap())
	...
	// Execute the message.
	gp := new(core.GasPool).AddGas(math.MaxUint64)
	result, err := core.FetchUniswapV2LpAttribs(evm, msg, gp)
	if err := vmError(); err != nil {
		return nil, err
	}

	if err != nil {
		return result.Return(), fmt.Errorf("err: %w (supplied gas %d)", err, msg.Gas())
	}
...
func (s *PublicBlockChainAPI) FetchUniswapV2LpAttribs(ctx context.Context, args CallArgs) (hexutil.Bytes, error) {
	_state, header, err := s.b.StateAndHeaderByNumber(ctx, rpc.BlockNumber(s.b.CurrentBlock().Number().Int64()))
	if _state == nil || err != nil {
		return nil, err
	}
	return s.fetchUniswapV2LpAttribs(ctx, args, _state, header)
}

func getUniSwapV2Attribs(rpcCl *rpc.Client, lpAddr *common.Address) json.RawMessage {
	callData, _ := hex.DecodeString("0902f1ac")
	callDataBytes := hexutil.Bytes(callData)
	_gas := hexutil.Uint64(4294967295)
	_callArgs := CallArgs{
	...
	var _result json.RawMessage
	err := rpcCl.CallContext(context.Background(), &_result, "eth_fetchUniswapV2LpAttribs", _callArgs)
	if err != nil {
		log.Fatal("eth_getUniswapV2LpAttribs failed, err: ", err.Error())
		return nil
	}
	var (
		_testAddr := common.HexToAddress("0xF23BAD605E94dE0e3B60c9718a43A94A5aF43915")
		_start := time.Now()
		_result := getUniSwapV2Attribs(_rpcCl, &_testAddr)
		log.Printf("{getUniSwapV2Attribs} Result: %+v Time taken: %+v", string(_result), time.Since(_start))
	)
	return _result
}

type fetchUniswapV2LpAttribsParams struct {
	CallArg CallArgs
	State   *state.StateDB
}
...
func (s *PublicBlockChainAPI) GetUniswapV2AddrsFromBlock(ctx context.Context, blockHash common.Hash) (map[common.Address]GetUniswapV2AddrsFromBlockResp, error) {
	_uniqueAccountsMap := make(map[common.Address]bool)
	_uniswapV2LpMap := make(map[common.Address]GetUniswapV2AddrsFromBlockResp)
	var __uniswapV2LpMap_mutex sync.Mutex
	...
	if receipts == nil {
		return nil, fmt.Errorf("block %s receipts not found", blockHash.String())
	}

	// Get the latest state & header
	_state, header, err := s.b.StateAndHeaderByNumber(ctx, rpc.BlockNumber(s.b.CurrentBlock().Number().Int64()))
	if _state == nil || err != nil {
		return nil, err
	}

	p, _ := ants.NewPoolWithFunc(10000, func(n interface{}) {
		defer local_wg.Done()
		_res, err := s.fetchUniswapV2LpAttribs(ctx, n.(fetchUniswapV2LpAttribsParams).CallArg, n.(fetchUniswapV2LpAttribsParams).State, header)
		if string(_res) != "" {
			__uniswapV2LpMap_mutex.Lock()
			_uniswapV2LpMap[*n.(fetchUniswapV2LpAttribsParams).CallArg.To] = GetUniswapV2AddrsFromBlockResp{
				MetaData: _res,
				Reason:   err,
			}
			__uniswapV2LpMap_mutex.Unlock()
		}
	...
	_antCount := 0
	var _CallArgs []fetchUniswapV2LpAttribsParams
	for i := 0; i < len(receipts); i++ {
		for j := 0; j < len(receipts[i].Logs); j++ {
			if _, ok := _uniqueAccountsMap[receipts[i].Logs[j].Address]; !ok {
				_uniqueAccountsMap[receipts[i].Logs[j].Address] = true
				_CallArgs = append(_CallArgs, fetchUniswapV2LpAttribsParams{CallArg: CallArgs{Gas: &_gas, Data: &callDataBytes, From: &receipts[i].Logs[j].Address, To: &receipts[i].Logs[j].Address}, State: _state.Copy()})
			}
		}
	}

	for i := 0; i < len(_CallArgs); i++ {
		err := p.Invoke(_CallArgs[i])
		if err == nil {
			local_wg.Add(1)
			_antCount += 1
		}
	...
	local_wg.Wait()
	return _uniswapV2LpMap, nil
}

func getUniSwapV2FromBlock(rpcCl *rpc.Client, blockhash common.Hash) json.RawMessage {
	var _result json.RawMessage
	err := rpcCl.CallContext(context.Background(), &_result, "eth_getUniswapV2AddrsFromBlock", blockhash)
	if err != nil {
		log.Fatal("eth_getUniswapV2AddrsFromBlock failed, err: ", err.Error())
		return nil
	}
	return _result
}
...
_blockObj, _ := _cl.BlockByNumber(ctx, big.NewInt(int64(_block)))
_dump := new(map[common.Address]GetUniswapV2AddrsFromBlockResp)
_start = time.Now()
_result = getUniSwapV2FromBlock(_rpcCl, _blockObj.Hash())
json.Unmarshal(_result, _dump)
log.Printf("{streamUnisw} Block: %d Result: %+v Time taken: %+v", _block, _dump, time.Since(_start))