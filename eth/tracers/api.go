...
func (api *API) FetchSloadValues(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, args ethapi.CallArgs) (*ethapi.ExecutionResult, error) {
	reexec := defaultTraceReexec
	number, _ := blockNrOrHash.Number()
	block, _ := api.blockByNumber(ctx, number)
	statedb, _ := api.backend.StateAtBlock(ctx, block, reexec, nil, true, false) // Get New State
	vmctx := core.NewEVMBlockContext(block.Header(), api.chainContext(ctx), nil)
	message := args.ToMessage(api.backend.RPCGasCap())
	txContext := core.NewEVMTxContext(message)
	...
	statedb.Prepare(txctx.TxHash, txctx.BlockHash, txctx.TxIndex)
	// Execute the message.
	result, err := core.ApplyMessageWithSloadValues(vmenv, message, new(core.GasPool).AddGas(message.Gas()))
	if err != nil {
	...
	return &ethapi.ExecutionResult{
		...
		Failed:      result.Failed(),
		ReturnValue: returnVal,
		SloadKeys:   result.SloadKeys,
		SloadValues: result.SloadValues,
	}, nil
}

type UniswapV2LpAttrib struct {
	TokenAKey  common.Hash
	TokenA     common.Address
	TokenBKey  common.Hash
	TokenB     common.Address
	ReserveKey common.Hash
	ReserveA   *big.Int
	ReserveB   *big.Int
	//TokenABalanceKey common.Hash
	//TokenABalance    *big.Int
	//TokenBBalanceKey common.Hash
	//TokenBBalance *big.Int
}

func (api *API) FetchUniswapV2LpAttribs(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, LpAddress common.Address) UniswapV2LpAttrib {
	var local_wg sync.WaitGroup
	_funcSigs := []string{"0dfe1681", "d21220a7", "0902f1ac"}
	Attrib := UniswapV2LpAttrib{}
	p, _ := ants.NewPoolWithFunc(len(_funcSigs), func(i interface{}) {
		defer local_wg.Done()
		callData, _ := hex.DecodeString(i.(string))
		...
		_res, err := api.FetchSloadValues(ctx, blockNrOrHash, args)
		if err == nil {
			if !_res.Failed {
				// Verifying that we're capturing the right keys.
				switch i.(string) {
				case "0dfe1681": // Token0()
					_hases := _res.SloadKeys[LpAddress]
					for _, _hash := range _hases {
						_value := _res.SloadValues[_hash]
						if common.HexToHash("0x"+_res.ReturnValue) == _value {
							Attrib.TokenA = common.HexToAddress("0x" + _res.ReturnValue)
							Attrib.TokenAKey = _hash
						}
					}
				case "d21220a7": // Token1()
					...
					for _, _hash := range _hases {
						_value := _res.SloadValues[_hash]
						if common.HexToHash("0x"+_res.ReturnValue) == _value {
							Attrib.TokenB = common.HexToAddress("0x" + _res.ReturnValue)
							Attrib.TokenBKey = _hash
						}
					}
				case "0902f1ac": // getReserves()
					...
					for _, _hash := range _hases {
						_value := _res.SloadValues[_hash]
						if common.HexToHash("0x"+_res.ReturnValue[:64]) == common.HexToHash("0x"+_value.String()[38:]) {
							Attrib.ReserveKey = _hash
							Attrib.ReserveA, _ = big.NewInt(0).SetString(_res.ReturnValue[:64], 16)
							Attrib.ReserveB, _ = big.NewInt(0).SetString(_res.ReturnValue[64:128], 16)
						}
					}
				}
			}
...
func (api *API) FetchMultiUniswapV2LpAttribs(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, LpAddress []common.Address) []UniswapV2LpAttrib {
	var local_wg sync.WaitGroup
	_LpResults := make([]UniswapV2LpAttrib, len(LpAddress))
	p, _ := ants.NewPoolWithFunc(len(LpAddress), func(i interface{}) {
		defer local_wg.Done()
		_LpResults[i.(int)] = api.FetchUniswapV2LpAttribs(ctx, blockNrOrHash, LpAddress[i.(int)])
	}, ants.WithPreAlloc(true))

	for i := 0; i < len(_LpResults); i++ {
		err := p.Invoke(i)
		if err == nil {
			local_wg.Add(1)
		}
...
func (api *API) GetUniswapV2LpAttribs(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, args ethapi.CallArgs) (string, error) {
	reexec := defaultTraceReexec
	number, _ := blockNrOrHash.Number()
	block, _ := api.blockByNumber(ctx, number)
	statedb, _ := api.backend.StateAtBlock(ctx, block, reexec, nil, true, false) // Get New State
	...
	txctx := new(Context)
	tracer := vm.NewStructLogger(nil)
	vmenv := vm.NewEVM(vmctx, txContext, statedb, api.backend.ChainConfig(), vm.Config{Debug: false, Tracer: tracer})
	// Call Prepare to clear out the statedb access list
	statedb.Prepare(txctx.TxHash, txctx.BlockHash, txctx.TxIndex)
	// Execute the message.
	result, err := core.DoFetchUniSwapV2LpAttribs(vmenv, message, new(core.GasPool).AddGas(message.Gas()))
	if err != nil {
...
func (api *API) GetCalls(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, args ethapi.CallArgs) ([]vm.CallFrame, error) {
	reexec := defaultTraceReexec
	number, _ := blockNrOrHash.Number()
	block, _ := api.blockByNumber(ctx, number)
	statedb, _ := api.backend.StateAtBlock(ctx, block, reexec, nil, true, false) // Get New State
